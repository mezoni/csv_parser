    ...
    if (state.ok) {
      ...
      while (true) {
        ...
        if ($13.isEmpty) {
          ...
          state.ok = state.pos < state.input.length &&
              state.input.codeUnitAt(state.pos) == $13.codeUnitAt(0) &&
              state.input.startsWith($13, state.pos);
          if (state.ok) {
            state.pos += $13.length;
          } else {
            state.fail(ErrorExpectedTags([$13]));
          }
        }
        if (state.ok) {
          $10 = false;
          ...
          if (state.ok) {
            ...
            $9 = endEvent<String>(CsvExParserEvent.fieldEvent, $9, state.ok);
            if (state.ok) {
              $4 = $9;
            }
          }
    ...
    void $1() {
      ...
      while (true) {
        if ($4 == 0) {
          ...
          $11 &= ~0x4 & 0xffff;
          $6 = endEvent<String>(CsvExParserEvent.fieldEvent, $6, state.ok);
          $11 &= ~0x8 & 0xffff;
          if (!state.ok) {
            break;
          }
        ...
        if ($4 == 1) {
          ...
          if ($15 == 0) {
            ...
            final $19 = _separator;
            if (state.pos + $19.length - 1 < $18.end || $18.isClosed) {
              matchLiteralAsync(state, $19, ErrorExpectedTags([$19]));
            } else {
              $18.sleep = true;
              $18.handle = $1;
              return;
          ...
          if ($15 == 2) {
            ...
            $11 &= ~0x40 & 0xffff;
            $14 = endEvent<String>(CsvExParserEvent.fieldEvent, $14, state.ok);
            $11 &= ~0x80 & 0xffff;
            $15 = -1;
          }
          if (state.ok) {
  ...
  String? parseText(State<String> state) {
    ...
    while (true) {
      ...
      if (state.ok) {
        ...
        ParseError? error;
        if ($$ == _separatorChar) {
          error = const ErrorUnexpectedCharacter();
        }
        if (error != null) {
          if ($6 <= pos) {
            state.failPos = $6;
