class CsvExParser {
    ...
    if (!state.ok) {
      ...
      while (true) {
        ...
        if ($8.isEmpty) {
          ...
          state.ok = state.input.startsWith($8, state.pos);
          if (state.ok) {
            state.pos += state.input.count;
          } else {
  ...
  List<List<String>>? parseRows(State<StringReader> state) {
    ...
    if (state.ok) {
      ...
      state.ok = state.pos < state.input.length;
      if (state.ok) {
        final $19 = state.input.readChar(state.pos);
        state.ok = $19 == 10 || $19 == 13;
        if (state.ok) {
          state.pos += state.input.count;
        }
      }
      ...
      if (!state.ok) {
        // '\r\n'
        const $17 = '\r\n';
        matchLiteral(state, $17, const ErrorExpectedTags([$17]));
      }
      state.ok = true;
      if (state.ok) {
        $0 = $2;
...
enum CsvExParserEvent { fieldEvent, rowEvent }
