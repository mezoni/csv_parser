...
List<int>? _chars(State<String> state) {
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      state.ok = false;
      final $5 = state.pos;
      if ($5 < source.length && source.codeUnitAt($5) == 34) {
        state.ok = source.startsWith('""', $5);
      }
      if (state.ok) {
        state.pos += 2;
        $4 = '""';
      } else {
        state.error = ErrExpected.tag($5, const Tag('""'));
      }
      if (state.ok) {
        $2 = 34;
      }
      ...
      final $7 = state.error;
      state.error = ErrCombined(state.pos, [$3, $7]);
      break;
    }
    if (!state.ok) {
      state.ok = true;
...
String? _string(State<String> state) {
  ...
  if (state.ok) {
    ...
    if (state.ok) {
      final v = $4!;
      $3 = String.fromCharCodes(v);
    }
    if (state.ok) {
      String? $5;
      final $6 = state.pos;
      if ($6 < source.length && source.codeUnitAt($6) == 34) {
        state.pos++;
        state.ok = true;
        $5 = '"';
      }
      if ($5 == null) {
        state.ok = false;
        state.error = ErrExpected.tag($6, const Tag('"'));
      }
      if (state.ok) {
        $0 = $3!;
      }
